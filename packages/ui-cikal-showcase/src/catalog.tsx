...
export function CikalCatalog() {
  ...
  // Bronze is light-only — reset dark when switching to it
  React.useEffect(() => {
    if (isBronze) setDark(false)
  }, [isBronze])
  ...
  return (
    <div className={cn(
      `tier-${tier} brand-cikal bg-[var(--background)] text-[var(--foreground)] transition-colors duration-200`,
      dark && "dark"
    )}>
      <main className="min-h-screen px-4 py-6 sm:px-10 sm:py-10 max-w-4xl mx-auto">
        ...
        <header className="mb-10">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <h1 className="typo-hero">ui-cikal-catalog</h1>
            ...
            <div className="flex flex-col items-end gap-3">
              <TierSwitcher />
              <div className={cn("flex items-center gap-2", isBronze && "opacity-40")}>
                <SunIcon className="text-[var(--muted-foreground)]" />
                <T.Switch
                  id="dark-toggle"
                  checked={dark}
                  onCheckedChange={isBronze ? () => {} : setDark}
                  disabled={isBronze}
                  aria-label="Toggle dark mode"
                />
                <MoonIcon className="text-[var(--muted-foreground)]" />
                {isBronze && (
                  <span className="typo-caption text-[var(--muted-foreground)]">Light only</span>
                )}
              </div>
              <p className="typo-caption text-[var(--muted-foreground)]">
                Toggle tier and theme
              </p>
            </div>
          </div>
        </header>
