...
function NavBar({ logo, brandName, showBrandName, links = [], user, onLogin, onMenuClick, className }: NavBarProps) {
  ...
  return (
    <nav
      data-slot="nav-bar"
      className={cn(
        "sticky top-0 z-10 flex flex-col border-b border-[var(--border)] bg-[var(--background)]/80 backdrop-blur-sm shadow-[var(--shadow-sm)]",
        className
      )}
    >
      ...
      {!onMenuClick && links.length > 0 && (
        <div
          ...
          className={cn(
            "grid overflow-hidden border-t border-[var(--border)] transition-[grid-template-rows] duration-300 ease-in-out sm:hidden",
