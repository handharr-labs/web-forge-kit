/**
 * `@handharr-labs/forge-auth/server` — server-only entrypoint.
 *
...
// Re-export the port types apps type against, so `@handharr-labs/forge-auth/server`
// is a single import surface (they still originate in `@handharr-labs/forge-core`).
export type {
  AuthGateway,
  AuthProviderId,
  AuthProvisioner,
